...
def find_train_and_test_index(chunk_path, scene_name=None, num_context_views=5,
    ...
    for example in chunk:
        ...
        if num_context_views + num_target_views > n_views:
            continue
        ...
        if render_video:
            ...
            if len(index_target_all) < num_target_views:
                continue

            if uniform_sample:
                index_target_select = [(index_context[i] + index_context[i+1]) // 2 for i in range(len(index_context) - 1)]
            else:
...
def generate_index_file(args):
    ...
    if args.train_set:
        out_dir = f"assets/re10k_train_set"
        os.makedirs(out_dir, exist_ok=True)
        data_dir = "datasets/re10k/train"
    else:
        out_dir = f"assets/re10k_evaluation"
        os.makedirs(out_dir, exist_ok=True)
        data_dir = "datasets/re10k/test"
    ...
    if args.start_frame is not None:
        if args.render_video:
            save_file = f"re10k_start_{args.start_frame}_distance_{args.frame_distance}_ctx_{n_ctx}v_video.json"
        else:
            save_file = f"re10k_start_{args.start_frame}_distance_{args.frame_distance}_ctx_{n_ctx}v_tgt_{n_tgt}v.json"
    else:
        save_file = f"re10k_ctx_{n_ctx}v_tgt_{n_tgt}v.json"
