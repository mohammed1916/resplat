...
class EncoderReSplat(Encoder[EncoderReSplatCfg]):
    def __init__(self, cfg: EncoderReSplatCfg) -> None:
        ...
        if self.cfg.latent_downsample == 4:
            feature_upsampler_channels = model_configs[self.cfg.monodepth_vit_type][
                'in_channels'] // 4 + 128 + 64 + 96 + 128
        elif self.cfg.latent_downsample == 2:
            if self.cfg.lowest_feature_resolution == 8:
                feature_upsampler_channels = model_configs[self.cfg.monodepth_vit_type][
                    'in_channels'] // 64 * 4 + 128 // 16 + 64 + 96 // 4 + 128 // 16
            else:
                feature_upsampler_channels = model_configs[self.cfg.monodepth_vit_type][
                    'in_channels'] // 64 * 4 + 128 // 4 + 64 + 96 + 128 // 4
            ...
            if self.cfg.fixed_latent_size:
                ...
                feature_upsampler_channels = model_configs[
                    self.cfg.monodepth_vit_type]['in_channels'] + 128 + 64 + 96 + 128
        else:
        ...
        if self.cfg.num_refine > 0:
            ...
            if self.cfg.fixed_latent_size:
                in_channels = (num_gaussian_parameters + 3) * \
                    self.cfg.init_gaussian_multiple + self.state_channels + 3 * 4 ** 2
            else:
                in_channels = (num_gaussian_parameters + 3) * self.cfg.init_gaussian_multiple + \
                    self.state_channels + 3 * self.cfg.latent_downsample ** 2
            ...
            if self.cfg.refine_same_num_points:
                in_channels = (num_gaussian_parameters + 3) + \
                    self.state_channels + resnet_channels
            else:
                in_channels = (num_gaussian_parameters + 3) * \
                    self.cfg.init_gaussian_multiple + self.state_channels + resnet_channels
            ...
            if self.cfg.init_gaussian_multiple == 4:  # re10k
                ...
                self.update_rgb_error_proj = nn.Sequential(
                    nn.Linear(3 * self.cfg.latent_downsample **
                              2, resnet_channels),
                    nn.LayerNorm(resnet_channels)
                )
    ...
    def forward(
        ...
        if self.cfg.depth_pred_half_res:
            ...
            half_img = F.interpolate(
                half_img, scale_factor=0.5, mode='bilinear', align_corners=True)
            half_img = rearrange(
                half_img, "(b v) c h w -> b v c h w", b=b, v=v)
            ...
            for key in results_dict.keys():
                ...
                if key != 'depth_preds':
                    for i in range(len(results_dict[key])):
                        results_dict[key][i] = F.interpolate(
                            results_dict[key][i], scale_factor=2, mode='bilinear', align_corners=True)
        ...
        mono_features = [F.interpolate(x, size=(
            h // 16, w // 16), mode='bilinear', align_corners=True) for x in results_dict['raw_mono_features']]
        if self.cfg.fixed_latent_size:
            scale_factor = 4
            mono_features = [F.pixel_shuffle(
                x, upscale_factor=scale_factor) for x in mono_features]
            # channel: 384 / 16 * 4
            mono_features = torch.cat(mono_features, dim=1)

            if self.cfg.latent_downsample == 8:
                mono_features = F.interpolate(
                    mono_features, scale_factor=0.5, mode='bilinear', align_corners=True)
        else:
            if self.cfg.latent_downsample == 4:
                scale_factor = 4
                mono_features = [F.pixel_shuffle(
                ...
                mono_features = [F.pixel_shuffle(
                    x, upscale_factor=scale_factor) for x in mono_features]
                mono_features = torch.cat(
                    mono_features, dim=1)  # channel: 384 / 4 * 4
            else:
                raise NotImplementedError
        ...
        if self.cfg.latent_downsample == 2:
            ...
            if self.cfg.lowest_feature_resolution == 8:
                # 1/2, 1/4, 1/8
                cnn_features[1] = F.pixel_shuffle(
                    cnn_features[1], upscale_factor=2)
                cnn_features[2] = F.pixel_shuffle(
                    cnn_features[2], upscale_factor=4)
                # 64 + 96 // 4 + 128 // 16 = 96
                ...
                # 1/2, 1/2, 1/4
                cnn_features[2] = F.pixel_shuffle(
                    cnn_features[2], upscale_factor=2)
                # 64 + 96 + 128 // 4
                cnn_features = torch.cat(cnn_features, dim=1)
            ...
            for i in range(len(cnn_features)):
                cnn_features[i] = F.interpolate(cnn_features[i], size=(
                    target_h, target_w), mode='bilinear', align_corners=True)
            cnn_features = torch.cat(cnn_features, dim=1)

            mv_features = results_dict["features_mv"][0]

            if mv_features.shape[-2] != target_h or mv_features.shape[-1] != target_w:
                mv_features = F.interpolate(mv_features, size=(
                    target_h, target_w), mode='bilinear', align_corners=True)
        ...
        if self.cfg.fixed_latent_size:
            if self.cfg.latent_downsample == 8:
                img_unshuffle = F.interpolate(
                    img_unshuffle, scale_factor=0.5, mode='area')

            img_unshuffle = F.pixel_unshuffle(
                img_unshuffle, downscale_factor=4)
        else:
            img_unshuffle = F.pixel_unshuffle(
                img_unshuffle, downscale_factor=self.cfg.latent_downsample)
        # depth is in the full resolution, downsample to latent depth
        if self.cfg.depth_pred_half_res:
            latent_depth = F.interpolate(
                depth, scale_factor=1./(self.cfg.latent_downsample // 2), mode='bilinear', align_corners=True)
        else:
            if self.cfg.no_upsample_depth:
                ...
                latent_depth = F.interpolate(
                    depth, scale_factor=1./self.cfg.latent_downsample, mode='bilinear', align_corners=True)
        ...
        origins, directions = get_world_rays(
            tmp_coords, tmp_extrinsics, tmp_intrinsics)
        point_cloud = origins + directions * tmp_depth

        offset = torch.tensor(
            [v * h * w * i for i in range(1, b + 1)]).to(depth.device)
        ...
        with torch.amp.autocast(device_type='cuda', enabled=self.cfg.pt_head_amp, dtype=torch.bfloat16):
            ...
            pt_output = self.pt(
                (point_cloud, tmp_feature, offset), **pt_kwargs)
            out = tmp_feature + pt_output

            condition_features = rearrange(
                out, "(bv h w) c -> bv c h w", h=h, w=w)

        with torch.amp.autocast(device_type='cuda', enabled=self.cfg.pt_head_amp, dtype=torch.bfloat16):
            out = self.gaussian_head(out)

        point_cloud = rearrange(
            point_cloud, "(b v h w) c -> b v (h w) () () c", b=b, v=v, h=h, w=w)
            ...
            if self.cfg.init_gaussian_multiple == 4:
                if self.cfg.latent_downsample == 4:
                    ...
                    depths = F.interpolate(
                        depth, scale_factor=0.5, mode='bilinear', align_corners=True)
                elif self.cfg.latent_downsample == 8:
                    depths = F.interpolate(
                        depth, scale_factor=0.25, mode='bilinear', align_corners=True)
                elif self.cfg.latent_downsample == 2:
                    depths = depth
                else:
                    raise NotImplementedError
        ...
        if repeat > 1:
            ...
            rotations_unnorm = rearrange(
                rotations_unnorm, "b v (h w) (c x y) -> b v (h x w y) c", h=latent_h, w=latent_w, x=r, y=r)
            scales = rearrange(
                scales, "b v (h w) (c x y) -> b v (h x w y) c", h=latent_h, w=latent_w, x=r, y=r)
            opacities_raw = rearrange(
                opacities_raw, "b v (h w) (c x y) -> b v (h x w y) c", h=latent_h, w=latent_w, x=r, y=r)
            offset = rearrange(
                offset, "b v (h w) (c x y) -> b v (h x w y) c", h=latent_h, w=latent_w, x=r, y=r)
            sh = rearrange(sh, "b v (h w) (c x y) -> b v (h x w y) c",
        ...
        sh_input_images = rearrange(
            context["image"], "b v c h w -> (b v) c h w")
        if self.cfg.latent_downsample == 4 and self.cfg.init_gaussian_multiple == 4:
            sh_input_images = F.interpolate(
                sh_input_images, scale_factor=0.5, mode='area')
        elif self.cfg.latent_downsample == 4 and self.cfg.init_gaussian_multiple == 16:
            pass
        elif self.cfg.latent_downsample == 2 and self.cfg.init_gaussian_multiple == 4:
            pass
            ...
            sh_input_images = F.interpolate(
                sh_input_images, scale_factor=0.25, mode='area')
        elif self.cfg.latent_downsample == 8 and self.cfg.init_gaussian_multiple == 16:
            sh_input_images = F.interpolate(
                sh_input_images, scale_factor=0.5, mode='area')
        else:
            sh_input_images = F.interpolate(
                sh_input_images, scale_factor=1./self.cfg.latent_downsample, mode='area')

        sh_input_images = rearrange(
            sh_input_images, "(b v) c h w -> b v c h w", b=b, v=v)
        ...
        rotations = rotations_unnorm / \
            (rotations_unnorm.norm(dim=-1, keepdim=True) + 1e-8)

        # Create world-space covariance matrices.
        covariances = build_covariance(scales, rotations)
        c2w_rotations = context["extrinsics"][...,
                                              # [B, V, 1, 3, 3]
                                              :3, :3].unsqueeze(2)
        covariances = c2w_rotations @ covariances @ c2w_rotations.transpose(
            -1, -2)

        # means
        origins, directions = get_world_rays(xy_ray,
                                             context["extrinsics"].unsqueeze(
                                                 2).unsqueeze(2),
        ...
        # [B, V, H*W, 3]
        sh_input_images = rearrange(
            sh_input_images, "b v c h w -> b v (h w) c")
        ...
        gaussians = Gaussians(
            ...
            rotations_unnorm=rearrange(
                rotations_unnorm, "b v r wxyz -> b (v r) wxyz")
        )
    ...
    def forward_update(self,
        ...
        # [B, N, 4]
        prev_rotations_unnorm = prev_gaussians.rotations_unnorm.detach()
        # before sigmoid, epe is necessary, otherwise might be nan
        prev_opacities_raw = torch.logit(
            prev_gaussians.opacities.detach(), eps=1e-6)  # [B, N]
        ...
        if self.cfg.init_gaussian_multiple == 4 and self.cfg.refine_same_num_points:
            state = F.interpolate(state, scale_factor=2,
                                  mode='bilinear', align_corners=True)
        elif self.cfg.init_gaussian_multiple == 16 and self.cfg.refine_same_num_points:
            state = F.interpolate(state, scale_factor=4,
                                  mode='bilinear', align_corners=True)
        else:
            pass
        ...
        if self.training and self.cfg.train_min_refine > 0 and self.cfg.train_max_refine > 0:
            num_refine = np.random.randint(
                self.cfg.train_min_refine, self.cfg.train_max_refine + 1)
        ...
        for i in range(num_refine):
                ...
                for feat in features:
                    if feat.shape[-2:] != (latent_h, latent_w):
                        feat = F.interpolate(feat, size=(
                            latent_h, latent_w), mode='bilinear', align_corners=True)
            ...
            input_render_error = rearrange(
                corr, "(b v) c h w -> b (v h w) c", b=b, v=v)

            # include both feature error and image error
            rgb_render_error = input_render.color - context["image"]
            rgb_render_error = rearrange(
                rgb_render_error, "b v c h w -> (b v) c h w")

            if self.cfg.init_gaussian_multiple != 4:  # re10k 256x256, no downsample
                rgb_render_error = F.pixel_unshuffle(
                    rgb_render_error, downscale_factor=self.cfg.latent_downsample)

            rgb_render_error = rearrange(
                # [B, N, C]
                rgb_render_error, "(b v) c h w -> b (v h w) c", b=b, v=v)
            ...
            if self.cfg.init_gaussian_multiple == 4 and not self.cfg.refine_same_num_points:
                ...
                point_cloud = rearrange(
                    point_cloud, "b v h w c -> (b v h w) c")
            ...
            offset = torch.tensor(
                [k * tmp_batch_size for k in range(1, b + 1)]).to(state.device)
            ...
            if self.cfg.init_gaussian_multiple == 4 and not self.cfg.refine_same_num_points:
                ...
                tmp_gaussian = rearrange(
                    prev_gaussians_concat, "b n c -> (b n) c")
            ...
            with torch.amp.autocast(device_type='cuda', enabled=self.cfg.use_amp, dtype=torch.bfloat16):
                for blk in self.update_error_attn:
                    if self.cfg.refine_same_num_points:
                        ...
                        input_render_error = blk(
                            input_render_error, v=v, h=h, w=w)
                    else:
                        input_render_error = blk(input_render_error,
                                                 v=v,
                                                 h=latent_h, w=latent_w)

            tmp_render_error = rearrange(
                input_render_error, "b n c -> (b n) c")

            with torch.amp.autocast(device_type='cuda', enabled=self.cfg.pt_update_amp, dtype=torch.bfloat16):
                concat = torch.cat(
                    (tmp_gaussian, tmp_state, tmp_render_error), dim=-1)
                ...
                if self.cfg.use_checkpointing or self.cfg.recurrent_use_checkpointing:
                    def recurrent_chunk(tmp_state, tmp_gaussian, tmp_render_error, point_cloud, offset,
                        ...
                        concat = torch.cat(
                            (tmp_gaussian, tmp_state, tmp_render_error), dim=-1)
                        pxo = self.update_module[0](
                            [point_cloud, concat, offset])
                        tmp_state = self.update_module[1](
                            pxo, **refine_pt_kwargs) + tmp_state
                        return tmp_state
            ...
            delta_gaussians = rearrange(
                delta_gaussians, "(b n) c -> b n c", b=b)
            ...
            if self.cfg.init_gaussian_multiple > 1 and not self.cfg.refine_same_num_points:
                delta_means = rearrange(
                    delta_means, "b n (c k) -> b (n k) c", k=repeat)
                delta_scales = rearrange(
                    delta_scales, "b n (c k) -> b (n k) c", k=repeat)
                delta_rotations = rearrange(
                    delta_rotations, "b n (c k) -> b (n k) c", k=repeat)
                delta_opacities = rearrange(
                    delta_opacities, "b n (c k) -> b (n k) c", k=repeat)
                delta_shs = rearrange(
                    delta_shs, "b n (c k) -> b (n k) c", k=repeat)

            prev_means = (prev_means + delta_means)

            # clamp the scale
            prev_scales = (
                prev_scales + delta_scales).clamp(min=self.cfg.gaussian_adapter.clamp_min_scale)

            prev_rotations_unnorm = prev_rotations_unnorm + delta_rotations
            # normalize
            prev_rotations = prev_rotations_unnorm / \
                (prev_rotations_unnorm.norm(dim=-1, keepdim=True) + 1e-8)

            # Create world-space covariance matrices.
            covariances = build_covariance(prev_scales, prev_rotations)
            covariances = rearrange(
                covariances, "b (v hw) x y -> b v hw x y", v=v)
            c2w_rotations = context["extrinsics"][...,
                                                  # [B, V, 1, 3, 3]
                                                  :3, :3].unsqueeze(2)
            # [B, V, HW, 3, 3]
        ...
        return {
            ...
            # [BVHW, C], for sliding window global fusion
