...
def single_head_split_window_attention(
    ...
    if not (q.dim() == k.dim() == v.dim() == 3):
        ...
        if USE_PYTORCH_ATTN:
            # single head
            # [B, H, N, C]
            # q = q.view(b_new, 1, -1, c)
            # k = k.permute(0, 2, 1).contiguous().unsqueeze(1)
            # v = v.unsqueeze(1)
            # # print(q.shape)
            # attn_mask = attn_mask.repeat(b, 1, m).unsqueeze(1) if with_shift else None
            # out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask).squeeze(1)

            out = F.scaled_dot_product_attention(
                q.view(b_new, 1, -1, c),
                k.permute(0, 2, 1).contiguous().unsqueeze(1),
                v.unsqueeze(1),
                attn_mask=attn_mask.repeat(b, 1, m).unsqueeze(1) if with_shift else None
                ).squeeze(1)
        else:
        ...
        if USE_PYTORCH_ATTN:
            ...
            # [B, H, N, C]
            # q = q.view(b_new, 1, -1, c)
            # k = k.view(b_new, 1, -1, c)
            # v = v.view(b_new, 1, -1, c)
            # # print(q.shape)
            # attn_mask = attn_mask.repeat(b, 1, 1).unsqueeze(1) if with_shift else None
            # out = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask).squeeze(1)
        ...
        **kwargs,
        ...
        for i, layer in enumerate(self.layers):
            if self.use_checkpointing:
                ...
                concat0 = torch.utils.checkpoint.checkpoint(custom_forward, concat0, concat1)
            else:
                concat0 = layer(
                    concat0,
                    concat1,
