...
class ViewSamplerBoundedV2Cfg:
    ...
    extra_views_sampling_strategy: Optional[Literal["random", "farthest_point", "uniform"]] = "random"
...
class ViewSamplerBoundedV2(ViewSampler[ViewSamplerBoundedV2Cfg]):
    ...
    def sample(
        ...
        if total_num_views > 2:
            ...
            elif self.cfg.extra_views_sampling_strategy == 'uniform':
                all_views = torch.linspace(index_context_left, index_context_right, total_num_views, dtype=int)
                extra_views = all_views[1:-1].tolist()
            else:
                raise ValueError("Invalid extra views sampling strategy.")
