...
python -m src.main +experiment=dl3dv \
    ...
    checkpointing.pretrained_model=<view16_512x960_init_checkpoint> \
    checkpointing.no_strict_load=true \
    checkpointing.resume_update_module=<view8_512x960_refine_checkpoint> \
    wandb.project=dl3dv-view16-540x960 \
    output_dir=checkpoints/resplat/dl3dv-view16-540x960/base-refine
...
CUDA_VISIBLE_DEVICES=0 python -m src.main +experiment=dl3dv \
    ...
    dataset/view_sampler=evaluation \
    dataset.view_sampler.num_context_views=16 \
    dataset.view_sampler.index_path=assets/dl3dv_evaluation/dl3dv_start_0_distance_160_ctx_16v_video.json \
    dataset.image_shape=[540,960] \
    dataset.ori_image_shape=[540,960] \
    model.encoder.gaussian_adapter.gaussian_scale_max=3. \
    model.encoder.num_refine=2 \
    ...
    output_dir=results/resplat-dl3dv-view16-540x960-base-refine-rendervideo
